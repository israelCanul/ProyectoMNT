		    ...
		    function setMarkers(map, markers) {
		        ...
		        for (var i = 0; i < markers.length; i++) {
		            ...
		            var marker = new google.maps.Marker({
		                ...
		                map: map,
		                icon: "/img/mapPointsHoteles2.png",
		...
		<section id="hotel_search_results" class="rightSide">
			...
			<div class="jplist-panel box panel-top">
				...
				<div class="sortSearch">
					...
					<select id="controlSortHotels" class="jplist-select" data-control-type="sort-select" data-control-name="sort" data-control-action="sort" style="display:none">
						...
						<div class='hotelContent elementData'>
							<div class="title elementName">
								<?php if((float)$_h->attributes()->minAverPrice == 99999999 || (float)$_h->attributes()->minAverPrice == 0){ ?>
									<a  href='<?php echo $this->createUrl('hoteles/detalle', array('hotel' => $_h->attributes()->hotel_keyword) );?><?=$_getPrecioMax?>' title='<?php echo $_h->attributes()->name; ?>'><span class="titleHotel"><div style="display:none">z</div><?php echo Yii::app()->GenericFunctions->makeSinAcento(utf8_decode($_h->attributes()->name)); ?><span></a>
								<?}else{?>
									<a  href='<?php echo $this->createUrl('hoteles/detalle', array('hotel' => $_h->attributes()->hotel_keyword) );?><?=$_getPrecioMax?>' title='<?php echo Yii::app()->GenericFunctions->makeSinAcento(utf8_decode($_h->attributes()->name)); ?>'><span class="titleHotel"><?php echo Yii::app()->GenericFunctions->makeSinAcento(utf8_decode($_h->attributes()->name)); ?><span></a>
								<?}?>
								<br/>
							...
							<span class="smallLetter">
								<a href='<?php echo $this->createUrl('hoteles/detalle', array('hotel' => $_h->attributes()->hotel_keyword) );?><?=$_getPrecioMax?>' title='More information'><?php echo Yii::t("global","Más información"); ?></a>
							</span>
                    	...
                    	<div class='elementTax'>
							...
							<?php }else{ ?>
									...
									<div class="elementBook">
										<a class='book curved misc_select_btn_green' href='<?php echo $this->createUrl('hoteles/detalle', array('hotel' => $_h->attributes()->hotel_keyword) );?><?=$_getPrecioMax?>' title='BOOK'>BOOK</a>
									</div>
							<?php } ?>
