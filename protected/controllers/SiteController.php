...
class SiteController extends Controller
{
	...
	public function actionEnviamail(){
		...
		if(empty($correoExiste)){
			...
			$message =	$this->renderPartial('application.views.partials.newsletter', $_REQUEST, true);
			...
			if(!$mail->Send()) {
				...
				echo 'Thank you for subscribing. Soon you will receive information and our best deals!';
			}
			exit();
		}else{
			echo 'You already suscribe to our Newsletter.';
		}
	}
	...
	public function actionBuscar(){
		...
		foreach($destinos as $l){
			...
			if($pos === false){
			   // echo "La cadena '$findme' no fue encontrada en la cadena '$mystring'<br>";
			    ...
			    $strClave['destinos'][]=array('label'=>GenericFunctions::makeSinAcento($l["ciudad_nombre"]),'url'=>$url);
			}
		}
		...
		foreach($Hoteles as $l){
			...
			if($pos === false){
			    ...
			    $strClave['hoteles'][]=array('label'=>GenericFunctions::makeSinAcento($l["hotel_nombre"]),'url'=>$url);
			}			
		}
		
		
		$tours= file_get_contents(Yii::app()->params['api']."/restTours/destinations?lan=en");		
		...
		foreach ($tours as $key => $value) {
			...
			if($pos === false){
				...
				if($value->tipo=='destination'){
					$url=Yii::app()->params["baseUrl"]."toursByDest/".$value->clave.".html?tour_destination=".GenericFunctions::makeSinAcento($value->label)."&tipo=destination&cat=&dest=".$value->id."&TourId=&sup=&openTk=0&seg=&tour-Checkin=".$dateCheckin."&tour_adults=2&tour_child=0&action=&PHPSESSID=9369665a1a4cae8bfe830631a3c4039a&lan=en&moneda=".Yii::app()->params['currency'];
					$strClave['tours']['dest'][]=array('label'=>GenericFunctions::makeSinAcento($value->label),'url'=>$url);
				}
				if($value->tipo=='category'){
					...
					$strClave['tours']['cat'][]=array('label'=>GenericFunctions::makeSinAcento($value->label),'url'=>$url);
				}		
				if($value->tipo=='tour'){
					...
					$strClave['tours']['tour'][]=array('label'=>GenericFunctions::makeSinAcento($value->label),'url'=>$url);
				}
				if($value->tipo=='supplier'){
					...
					$strClave['tours']['sup'][]=array('label'=>GenericFunctions::makeSinAcento($value->label),'url'=>$url);
				}											
			}			
